class RecipesController < ApplicationController
  ...
  # GET /recipes.json
  def index
    @recipes = Recipe.where('lower(city) LIKE lower(?) OR lower(city) LIKE lower(?)', "%#{params[:search]}%", "%#{params[:search]}%").paginate(:page => params[:page], :per_page => 8)
  ...
  private
    ...
    def recipe_params
      params.require(:recipe).permit(:title, :price, :no_people, :description, :date, :time, :street, :city, :location, :image)
    end
end
